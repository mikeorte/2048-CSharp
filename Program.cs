...
namespace _2048_CSharp
{
    internal class Program
    {
        ...
        static void Main()
        {
            ...
            while (true)
            {
                ...
                if (IsValidMove(move))
                {
                    ...
                    if (HasEmptyCell(gameGrid))
                    {
                        PlaceRandomTile(gameGrid); // Adds new tile only if there is an empty cell
                    }
        ...
        static bool HasEmptyCell(int[,] gameGrid)
        {
            for (int row = 0; row < N; row++)
            {
                for (int col = 0; col < N; col++)
                {
                    ...
                    {
                        return true; // If any cell is empty, return true
                    }
                }
            }
